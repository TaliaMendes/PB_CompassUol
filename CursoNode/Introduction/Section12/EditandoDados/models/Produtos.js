...
class Produto {
  ...
  editProduct(id){
    //passa o id do porduto e passa o dado do corpo para atualizar,
    const produto = connection.db().collection('produtos').deleteOne({_id: ObjectId(id)}, {$set : this}) //'$set' : utilizado para passar os dados do corpo. 'this' passa todos os dados da classe(constructor)
    return
  }
}
